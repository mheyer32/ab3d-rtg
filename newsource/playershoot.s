...
Player1Shot: tst.w        PLR1_TimeToShoot
	beq.s        okcanfire
...
.itsaclick:  tst.b        p1_clicked
	beq          PLR1_nofire

.itsahold:   move.w       PLR1_angpos,d0
	move.l       #SineTable,a0
	lea          (a0,d0.w),a0
	move.w       (a0),tempxdir
...
notlinedup:  add.w        #64,a0
	bra          findclosestinline

outofline:   move.w       d1,targdist
...
.okcanshoot: move.w       8(a6),PLR1_TimeToShoot
...
FIREBULLETS: movem.l      a0/a1/d7/d0/a5,-(a7)
	jsr          GetRand
	...
	bra.s        .missplr
.hitplr:     movem.l      (a7)+,a0/a1/d7/d0/a5
	move.l       d0,-(a7)
	bsr          PLR1HITINSTANT
	move.l       (a7)+,d0
...
.nofurther:  movem.l      (a7)+,d0-d7/a0-a6
...
Player2Shot: tst.w        PLR2_TimeToShoot
	beq.s        okcanfire2
...
okcanfire2:  lea          PLR2_GunData,a6
	moveq        #0,d0
...
.itsaclick:  tst.b        p2_clicked
	beq          PLR2_nofire

.itsahold:   move.w       PLR2_angpos,d0
	move.l       #SineTable,a0
	lea          (a0,d0.w),a0
	move.w       (a0),tempxdir
	move.w       2048(a0),tempzdir
...
notlinedup2: add.w        #64,a0
	bra          findclosestinline2

outofline2:  move.w       d1,targdist
...
.okcanshoot: move.w       8(a6),PLR2_TimeToShoot
...
firefive:    move.l       PlayerShotData,a0
	move.w       #19,d1
.findonefree move.w       12(a0),d0
	blt.s        .foundonefree
	adda.w       #64,a0
	dbra         d1,.findonefree
...
.foundonefree
	...
	blt.s        .okdownspd
	move.w       #20*128,d0
.okdownspd:  cmp.w        #-20*128,d0
	bgt.s        .okupspd
	move.w       #-20*128,d0
.okupspd:    add.w        20(a6),d0
