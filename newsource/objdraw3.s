...
ObjDraw:		move.w	(a0)+,d0
				cmp.w	#1,d0
				blt.s	beforewat
				beq.s	afterwat
				bgt.s	fullroom

beforewat:		move.l	BEFOREWATTOP,ty3d
				move.l	BEFOREWATBOT,by3d
				bra.s	donetopbot

afterwat:		move.l	AFTERWATTOP,ty3d
				move.l	AFTERWATBOT,by3d
				bra.s	donetopbot

fullroom:		move.l	TOPOFROOM(pc),ty3d
...
; CACHE_ON d6
				...
				move.l	a3,a4
				move.w	#79,d7
emptytab:		move.l	#$80010000,(a3)+
				dbra	d7,emptytab

				moveq	#0,d0
insertanobj		move.w	(a1),d1
				blt		sortedall
...
sortedall:		move.l	#depthtable,a3
...
okrightside:	move.l	#objintocop,a1
				lea		(a1,d0.w*2),a1
...
.dontplotthisoneitsblack:
				...
				dbra	d4,.drawavertstrip
				move.w	(a7)+,d4
blankstrip:		dbra	d3,drawrightside
				bra.s	objbehind

SecThird:		move.l	d5,d1
				move.l	d5,d6
				move.w	d4,-(a7)
.drawavertstrip
				move.w	(a0,d1.w*2),d0
...
objbehind:		movem.l	(a7)+,d0-d7/a0-a6
				rts
...
doapoly:		move.w	#960,Left
				...
				bsr		putinlines
				bra.s	dontusegour
usegour:		bsr		putingourlines
dontusegour:	move.w	#104*4,linedir
				move.l	frompt,a6
...
.nobot			lea		(a4,d1.w*8),a4
				...
				bge.s	toobright
				move.w	#0,d1
toobright:		move.w	(a1,d1.w*2),d1
				asl.w	#3,d1
				move.l	#TexturePal,a1
				add.w	d1,a1

dopoly:			move.w	#0,offtopby
				move.l	a6,a3
				adda.w	(a2)+,a3
				move.w	(a4),d1
				cmp.w	objclipb,d1
...
gotholesin:		move.l	#TexturePal,a1
				neg.w	d1
				add.w	#14,d1
				bge.s	toobrighth
				move.w	#0,d1
toobrighth:		asl.w	#8,d1
				lea		(a1,d1.w*2),a1

dopolyh:		move.l	a6,a3
				adda.w	(a2)+,a3
				move.w	(a4),d1
				cmp.w	objclipb,d1
				bge		nodlh
...
drawpolh:		and.w	d1,d5
				move.b	(a0,d5.w*4),d0
				beq.s	dontplot
				move.w	(a1,d0.w*2),(a3)
dontplot:		adda.w	#104*4,a3
				add.w	a5,d3
				addx.l	d6,d5
				dbcs	d2,drawpol2h
				dbcc	d2,drawpolh
				bra.s	pastith
drawpol2h:		and.w	d1,d5
				move.b	(a0,d5.w*4),d0
				beq.s	dontplot2
				move.w	(a1,d0.w*2),(a3)
dontplot2:		adda.w	#104*4,a3
				add.w	a5,d3
				addx.l	d4,d5
				dbcs	d2,drawpol2h
				dbcc	d2,drawpolh
...
Spider_des:		INCBIN	robot.vec
Medi_des:		INCBIN	medipac.vec
...
boxang:			dc.w	0
				...
				ds.w	96*4
PolyBotTab:		ds.w	96*4
				ds.w	96*4
PolyTopTab:		ds.w	96*4
				ds.w	96*4
...
MARINE_FRAMES:
				...
				dc.w	(64*17)*4,0
				dc.w	(64*18)*4,0
MARINE_PAL:		INCBIN	"newmarine.pal"
TOUGHMARINE_FRAMES:
				dc.w	0,0
				dc.w	64*4,0
				dc.w	(64*2)*4,0
