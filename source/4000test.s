...
Team17ScreenWidth		equ	640
Team17ScreenHeight		equ	1700
Team17ScreenDepth		equ	1

;_LVOScreenPosition	EQU	-792


PIC_SIZE	equ 128004+(640*40)
TEMP_SIZE	equ 18500
...
docredits:
	...
	lea		DosName(pc),a1		; graphics.library
	CALLEXEC 	OldOpenLibrary		; Open gfx lib
	move.l		d0,_DOSBase			; store ptr
	tst.l		d0				; Failed to open GFX lib ?
	beq		QuitF				; yes, jump to end
...
; decompress pic
	...
	move.l		d0,jPlane1			; chip mem..



	move.l		TempPtr,d0			; source
	moveq		#0,d1
	move.l		jPlane1,a0			; destination
	add.l		#640*20,a0
	lea		WorkSpace,a1			; workspace
	lea		$0,a2				; dunno ?
	jsr		unLHA

	move.l		TempPtr(pc),a1
	move.l		#TEMP_SIZE,d0
	CALLEXEC	FreeMem
	...
	CALLEXEC 	OldOpenLibrary		; Open gfx lib
	move.l		d0,_GfxBase			; store ptr
	tst.l		d0				; Failed to open GFX lib ?
	beq		QuitCloseGfx			; yes, jump to end

	move.w		#%100000,$dff096
	move.l		d0,a1
	move.l		$26(a1),$dff080 	; restore system clist
	move.w		#%100000,$dff096

	lea		IntuiName(pc),a1		; intuition.library
	CALLEXEC	OldOpenLibrary		; Open it
	move.l		d0,_IntuitionBase		; Store ptr to library
	tst.l		d0
	beq		QuitCloseInt
	move.w		#%100000,$dff096

	move.w		#%100000,$dff096
	lea		Team17BitMap(pc),a0		; Our bitmap struct
	move.b		#Team17ScreenDepth,d0	; depth of screen
	move.w		#Team17ScreenWidth,d1	; width of screen
	move.w		#Team17ScreenHeight,d2	; height of screen
	CALLGRAF	InitBitMap			; Setup Bitmap Struct
	move.w		#%100000,$dff096

	lea		Team17NewScreen(pc),a0	; Open Logo screen
	lea		Team17TagList(pc),a1
	CALLINT	OpenScreenTagList
	move.l		d0,Team17ScreenBase		; store screenbase
	move.w		#%100000,$dff096

	add.l		#44,d0
	move.l		d0,a0				; sc_ViewPort
	move.l		d0,Team17ViewPort		; store value

	lea		Team17Palette(pc),a1		; Do Palette for logo
	move.l		#PaletteSize/2,d0
	CALLGRAF 	LoadRGB4			; Load Palette


	move.l		Team17ScreenBase(pc),a0
...
;	btst.b		#6,$bfe001				; tes mouse button
;	beq		out
...
Team17NewScreen
	dc.w	0						; LeftEdge
	dc.w	0						; TopEdge
	dc.w	Team17ScreenWidth				; Width	
	dc.w	Team17ScreenHeight				; Height
	dc.w	Team17ScreenDepth				; Depth
	dc.b	0						; DetailPen
	dc.b	0						; BlockPen
	dc.w	HIRES_KEY						; ViewModes
	dc.w	CUSTOMBITMAP|SCREENQUIET|SCREENBEHIND	; Type

	; SCREENQUIET is a new wb2 type which stops the titlebar/gadgets
	; being initialized

	dc.l	0		; Font
	dc.l	0		; DefaultTitle
	dc.l	0		; Gadgets	
	dc.l	Team17BitMap	; CustomBitMap
	dc.l	0,0


Team17BitMap
	dc.w	0		; Bytes Per Row
	dc.w	0		; Rows
	dc.b	0		; Flags
	dc.b	0		; Depth
	dc.w	0		; pad	
jPlane1
	dc.l	0               ; Planes[0] ptr to bitmap
	dc.l	0
