	...
	SECTION      main,CODE
	OPT          O+,W-,P=68020
	OUTPUT       GAMES:AB3D/jgabd

	INCDIR       PROJ:AB3D/INCLUDE/
	INCDIR       INCLUDE:
	INCDIR       MYINC:
...
Processor:   equ          2	            ;0/680x0 (= 0 is faster than 68000)
...
Start:	move.l       #$dff000,a6		; NB V. IMPORTANT: A6=CUSTOM BASE
	...
	moveq        #0,d0		;Disable D-cache. WHY!?!
	move.l       #CACRF_EnableD,d1
	EXECCALL     CacheControl
...
FONTADDRS:   dc.l         ENDFONT0,CHARWIDTHS0
	dc.l         ENDFONT1,CHARWIDTHS1
	dc.l         ENDFONT2,CHARWIDTHS2

ENDFONT0:    INCBIN       "endfont0"
CHARWIDTHS0: INCBIN       "charwidths0"
ENDFONT1:    INCBIN       "endfont1"
CHARWIDTHS1: INCBIN       "charwidths1"
ENDFONT2:    INCBIN       "endfont2"
CHARWIDTHS2: INCBIN       "charwidths2"
	EVEN
...
.addup:      addq         #1,d5
	...
	move.w       d5,d6
	move.w       d3,d1
.DONTPUTIN:  dbra         d0,.addup
	asr.w        #1,d1
	neg.w        d1
	add.w        #320,d1	      ; horiz pos of start x

NOTCENTRED:  move.w       d6,d7
DOACHAR:     moveq        #0,d2
	move.b       (a0)+,d2
	sub.w        #32,d2
	moveq        #0,d6
...
.fdup        move.w       d0,TXTCOLL
	add.w        #$121,d0
.wtframe:    btst         #INTB_VERTB,custom+intreqr
	beq.s        .wtframe
...
VBname	dc.b	"AB3DVBlank",0
KBname	dc.b	"AB3DKeyboard",0
...
initlevel:	move.l       LEVELGRAPHICS,a0
	move.l       12(a0),a1
	add.l        a0,a1
...
nomorethiszone:
	...
	move.l	d0,pos2RIGHT
	move.l       #emptyend,d0
	move.l	d0,Samp0endLEFT
	move.l	d0,Samp1endLEFT
	move.l	d0,Samp0endRIGHT
	move.l	d0,Samp1endRIGHT
...
CLOSETXT:    move.w       #$8f8,d0
	move.w       #7,d1

.fdup        move.w       d0,TXTCOLL
	sub.w        #$121,d0
.wtframe:    btst         #5,$dff000+intreqrl
	beq.s        .wtframe
...
waitfortop:  btst.b       #0,intreqrl(a6)
	beq.b        waitfortop
...
DrawDisplay: clr.b        fillscrnwater
...
itsanarc:    jsr          CurveDraw
	bra          polyloop

itsanobject: jsr          ObjDraw
	bra          polyloop
...
playgameover:
	move.l       #$dff000,a6
waitfortop2: btst.b       #0,intreqrl(a6)
	beq          waitfortop2
	move.w       #$1,intreq(a6)
...
playwelldone:
	move.l       #$dff000,a6
waitfortop3: btst.b       #0,intreqrl(a6)
	beq          waitfortop3
	move.w       #$1,intreq(a6)
...
* so no bottom clipping is needed.
	...
	divs         d4,d1	        ; first y
	divs         d5,d3	        ; second y
lineclipped: move.l       #rightsidetab,a3
	cmp.w        d1,d3
	beq          lineflat
	st           drawit
	bgt          lineonright
...
OtherInter:  move.w       #$0010,$dff000+intreq
	movem.l      d0-d7/a0-a6,-(a7)
	bra.s        justshake
