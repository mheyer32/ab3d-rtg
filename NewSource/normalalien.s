ItsANasty:   tst.b        NASTY
	bne          .yesnas
	move.w       #-1,12(a0)
	rts
.yesnas:     move.w       #$1f1f,14(a0)
...
.oknn:       add.b        d0,d1
	...
	move.w       12(a0),d2
	bge.s        .stillalive
.notthisone: move.w       12(a0),GraphicRoom(a0)
	rts
.stillalive: tst.b        numlives(a0)
	bgt          .notdying
...
.dyinganim:  dcb.w        11,33
	dcb.w        15,32


.notdying:   tst.b        17(a0)
	beq.s        .cantseeplayer
	tst.w        ThirdTimer(a0)
	ble          NastyAttack
...
.waitandsee: move.w       #25,FourthTimer(a0)
...
.canmove:    clr.b        wallbounce
	jsr          MoveObject
	movem.l      (a7)+,d0/a0/a1/a3/a4/d7
	move.b       StoodInTop,ObjInTop(a0)

.hitathing:  tst.b        hitwall
	beq.s        .nochangedir
	move.w       #-1,ObjTimer(a0)
.nochangedir move.l       objroom,a2
	move.w       (a2),12(a0)
	move.w       newx,(a1)
	move.w       newz,4(a1)
...
.noexplode:  movem.l      d0-d7/a0-a6,-(a7)
	sub.l        ObjectPoints,a1
	add.l        #ObjRotated,a1
	move.l       (a1),Noisex
	move.w       #200,Noisevol
...
.notdeadyet: clr.b        damagetaken(a0)
	movem.l      d0-d7/a0-a6,-(a7)
	sub.l        ObjectPoints,a1
	add.l        #ObjRotated,a1
	move.l       (a1),Noisex
...
.nohiss:     move.b       ObjInTop(a0),ViewerTop
...
.hitathing:  tst.b        GotThere
	...
	bra.s        .NoMunch
.OKtomunch:  move.w       #20,FourthTimer(a0)
	move.l       PLR1_Obj,a2
	add.b        #2,damagetaken(a2)

.NoMunch:    move.l       objroom,a2
	move.w       (a2),12(a0)
	move.w       newx,(a1)
	move.w       newz,4(a1)
...
.hitathing:  tst.b        GotThere
	...
	sub.w        d0,FourthTimer(a0)
	bra.s        .NoMunch2
.OKtomunch2: move.w       #20,FourthTimer(a0)
	move.l       PLR2_Obj,a2
	add.b        #2,damagetaken(a2)

.NoMunch2:   move.l       objroom,a2
	move.w       (a2),12(a0)
	move.w       newx,(a1)
	move.w       newz,4(a1)
