...
.mainloop:	bsr	WaitIntuiMessage		;Wait for message
...
.NewWindowTags:
	...
	dc.l WA_Activate,-1
	dc.l WA_IDCMP,IDCMP_RAWKEY|IDCMP_ACTIVEWINDOW|IDCMP_INACTIVEWINDOW
...
WaitIntuiMessage:	;Wait for an IDCMP message and store its class/code
	;Exit:	Msg_Class/d0 hold message class
	;	Msg_Code/d1  hold message code

	movem.l	d2-7/a0-6,-(a7)

.wi_loop:	move.l	Win_UP,a0		;Get intuition signal set for our window
	moveq	#0,d0
	moveq	#0,d1
	move.b	MP_SIGBIT(a0),d1
	bset	d1,d0

	move.l	d0,-(a7)		;Wait for a signal for our window
	EXECCALL	Wait
	and.l	(a7)+,d0
	beq.s	.wi_loop

	move.l	Win_UP,a0		;Get d0=message from userport
	CALLA6	GetMsg
	...
	CALLA6	ReplyMsg		;Reply to message
